...
int create_bacterium(LAMMPS* lmp, double x0, double y0, double z0, double angle, int type, int state, int tolerance, double vx = 0, double vy = 0, double vz = 0);
...
void run_and_tumble(LAMMPS* lmp, double t_run, double t_tumble);
...
//  sprintf(run,"dump WRITE mobile custom 100 dumps/dumpfile.*.txt x y mol type");
  sprintf(run,"dump WRITE mobile custom 100 dumps/dumpfile.*.txt x y q mol");
  lmp->input->one(run);
...
//  sprintf(run,"neigh_modify exclude group all deleted"); lmp->input->one(run); 
  ...
  sprintf(run,"fix CALLBACK all external pf/callback 1 1");
  lmp->input->one(run);
...
int create_bacterium(LAMMPS* lmp, double x0, double y0, double z0, double angle, int type, int state, int tolerance, double vx, double vy, double vz){
...
//  char line2[1024];
  ...
  in_cylinder = touch(lmp, ns, ne, Diameter);
...
int touch(LAMMPS* lmp, int ns, int ne, double radius){
  ...
  for(int i = ns; i <= ne; i++){
    ...
    x0 = x[n][0];
    y0 = x[n][1];
    if( (x0 < radius) || (x0 > (Lbox - radius)) || (y0 < radius) || (y0 > (Lbox-radius))){ s = -1; e = 1;} else {s = e = 0;}
    for(int a = s; a <= s; a++){
      for(int b = e; b <= e; b++){
...
//  x & y of the two short molecules
        x1 = unwrap_ns[0]+(-0.5*Diameter+1.5*Length_Molecule)*(dr[0]/norm)*1.0001;
        y1 = unwrap_ns[1]+(-0.5*Diameter+1.5*Length_Molecule)*(dr[1]/norm)*1.0001;
        x2 = unwrap_ns[0]+(-0.5*Diameter+0.5*Length_Molecule)*(dr[0]/norm)*1.0001;
        y2 = unwrap_ns[1]+(-0.5*Diameter+0.5*Length_Molecule)*(dr[1]/norm)*1.0001;
//        out = fopen("prima.txt","w"); write_positions(lmp,out,0); fclose(out);
// remove long bacterium
//        cout << "Bacterium type : " << Bacterium[m].type << "	" << endl;

// insert two short ones;
        new_ns = create_bacterium(lmp, x1, y1, 0, angle, 0, Bacterium[m].state, 2*NBeads_Molecule, vel[0],vel[1],vel[2]);
        new_ns2 = create_bacterium(lmp, x2, y2, 0, angle, 0, Bacterium[m].state, 2*NBeads_Molecule, vel[0],vel[1],vel[2]);
        if( (new_ns > 0) || (new_ns2 > 0) ){
          delete_bacterium(lmp,m);
          ...
          Bacterium[p].Torque = Bacterium[m].Torque;
          if(Bacterium[p].state == 0){
            Bacterium[p].t_transition = total_time -1.0/t_run*log(Xrandom());
          }else{
            Bacterium[p].t_transition = total_time -1.0/t_tumble*log(Xrandom());
          }
          sprintf(line,"group molecule_new id %d:%d", new_ns, new_ns+NBeads_Molecule-1); lmp->input->one(line);
          sprintf(line,"set group molecule_new charge %g", charge); lmp->input->one(line);
          sprintf(line,"group molecule_new delete"); lmp->input->one(line);
        }else{ // have created one bacterium, not to. We destroy it, and keep the father
          if(new_ns > 0){ p = 0; while(Bacterium[p].nstart != new_ns) p++; delete_bacterium(lmp,m);}
          if(new_ns2 > 0){ p = 0; while(Bacterium[p].nstart != new_ns) p++; delete_bacterium(lmp,m);}
        } 
      }
    }
  }
...
void create_template_molecule(LAMMPS* lmp, int me, int ngrow){
  ...
  for(int k = 0; k < ngrow; k++){
    ...
    for(int n = 0; n < N; n++){
      outfile << n+1 << "	" << 0.5*Diameter+(length-Diameter)*n/(N-1) << "	" << 0 << "	" << 0 << endl;
    }
    outfile << endl;
...
void force_callback(void *ptr, bigint ntimestep, int nlocal, int *id, double **x, double **f) {
  ...
  for(int m = 0; m < Nmax_bacteria; m++) if(Bacterium[m].exist > 0){
...
//void change_state(LAMMPS* lmp, int n){
//  if(Bacterium[n].state == 0){
//    Bacterium[n].state = 1;
//  }else if(Bacterium[n].state == 1){Bacterium[n].state = 0;
//  }
//}
