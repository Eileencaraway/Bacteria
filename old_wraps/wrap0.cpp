...
double t_stationary = 2;
double p_move_again = 0.1;
double p_detach = 0.01;

// probabilites
double p_sibiling_bounded = 0.6;

double Lbox,Frun,Torque,Kn,DiameterPush,viscosity,viscosity2;
...
//  sprintf(run,"dump WRITE mobile custom 100 dumps/dumpfile.*.txt x y mol type");
  sprintf(run,"dump WRITE mobile custom 200 dumps/dumpfile.*.txt x y q mol");
  lmp->input->one(run);
...
//  fixrigid = (FixRigidNVESmall *) lmp->modify->fix[ifix];
  ...
  lmp->input->one(run);
/*
  for(int m; m< Nmax_bacteria;m++){
    if(Bacterium[m].state == 2){
        sprintf(run,"fix VIS     stationary     viscous %g", viscosity2);
        lmp->input->one(run);
    }
  }
*/
...
void setup(LAMMPS* lmp, int me){
  ...
  viscosity2 = get_single_variable(lmp,"viscosity2");
