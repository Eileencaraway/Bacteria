...
void compute_velocity(LAMMPS* lmp){
  ...
  double vysum=0.0;
  double vzsum=0.0;
  double v_average,phi; //phi is a parameter
  for(int m = 0; m < Nmax_bacteria; m++){
    if(Bacterium[m].exist > 0){
      ns = Bacterium[m].nstart; // gloab id
      ne = Bacterium[m].nend;   // global id
      ...
      Vy=v[ie][1];
      Vy=v[ie][2];
      v1=sqrt(v[ie][0])**2+(v[ie][1])**2+(v[ie][2])**2;
      vsum+=v1;
      vxsum+=Vx;
      vysum+=Vy;
      vzsum+=Vz;
  ...
  phi=sqrt(abs(vxsum**2+vysum**2+vzsum**2))/(count*v_average);

  ofstream outfile("velocity.txt",iso::app);
