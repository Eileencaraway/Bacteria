...
double t_immobile = 0.005; // typical time before a mobile bacterium becomes immobile still need to tune the parameter. doesn't look right
double t_mobile = 0.01; // typical time before an immobile bacterium becomes mobile

bool psl = false;
bool change_motility = true;
...
//    cout << q+1 << "	" << num_bacteria << endl;
  ...
  int final_time = 3*60*60;  // we simulate 7h
  int total_step = final_time/dt_integration;
  double tau= Mesh_size/Velocity;
  int timestep_record = 2*int(tau/dt_integration);
